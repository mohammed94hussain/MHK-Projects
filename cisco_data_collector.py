
import getpass
import time
from netmiko import ConnectHandler
from openpyxl import Workbook, load_workbook

# Prompt for username and password
username = input("Enter your username: ")
password = getpass.getpass()

# Read IP addresses from IP.xlsx
try:
    wb_ip = load_workbook('IP.xlsx')
    sheet_ip = wb_ip.active
    # Assuming IPs start from A2, change if your file is different
    ips = [cell.value for cell in sheet_ip['A'][1:] if cell.value]
except FileNotFoundError:
    print("Error: IP.xlsx not found. Please create it and add IP addresses in column A, starting from row 2.")
    exit()

# Create a new workbook for the output
wb_db = Workbook()
sheet_db = wb_db.active
sheet_db.title = "DataBase"

# Write headers to the output file
headers = [
    "Hostname", "IP", "Port", "Description", "Status", "VLAN",
    "Neighbor", "Port Speed", "Link Uptime", "Device Serial",
    "Power Supply Serials", "Transceiver Info"
]
sheet_db.append(headers)


# Loop through each IP address
for ip in ips:
    print(f"Connecting to {ip}...")
    device = {
        'device_type': 'cisco_ios',
        'host': ip,
        'username': username,
        'password': password,
    }

    try:
        net_connect = ConnectHandler(**device)
        net_connect.enable()

        # Get hostname
        hostname = net_connect.find_prompt()[:-1]

        # Get device serial and power supply serials
        show_inventory = net_connect.send_command('show inventory', use_textfsm=True)
        device_serial = ""
        power_supply_serials = []
        if isinstance(show_inventory, list):
            for item in show_inventory:
                if "chassis" in item.get('name','').lower():
                    device_serial = item.get('sn', 'N/A')
                if "power supply" in item.get('descr','').lower():
                    power_supply_serials.append(item.get('sn', 'N/A'))

        # Get interface status
        show_int_status = net_connect.send_command('show interfaces status', use_textfsm=True)

        # Get LLDP neighbors
        show_lldp_neighbors = net_connect.send_command('show lldp neighbors detail', use_textfsm=True)
        neighbors = {}
        if isinstance(show_lldp_neighbors, list):
            for neighbor in show_lldp_neighbors:
                neighbors[neighbor['local_interface']] = neighbor.get('neighbor_system_name', 'N/A')

        # Get interface descriptions
        show_int_desc = net_connect.send_command('show interfaces description', use_textfsm=True)
        descriptions = {item['port']: item['desc'] for item in show_int_desc}

        # Get transceiver information
        show_int_transceiver = net_connect.send_command('show interfaces transceiver detail', use_textfsm=True)
        transceivers = {}
        if isinstance(show_int_transceiver, list):
            for item in show_int_transceiver:
                # This command gives a lot of detail, so we format it nicely
                transceivers[item['port']] = f"Type: {item.get('type', 'N/A')}, SN: {item.get('serial_num', 'N/A')}, RX Power: {item.get('receive_pwr', 'N/A')}"

        # Get interface uptime
        show_interfaces = net_connect.send_command('show interfaces', use_textfsm=True)
        interface_uptimes = {iface['interface']: iface.get('link_uptime', 'N/A') for iface in show_interfaces}

        # Process each interface
        for interface in show_int_status:
            port = interface['port']
            description = descriptions.get(port, 'N/A')
            status = interface['status']
            vlan = interface['vlan']
            neighbor = neighbors.get(port, 'N/A')
            port_speed = interface.get('speed', 'N/A')
            link_uptime = interface_uptimes.get(port, 'N/A')
            transceiver_info = transceivers.get(port, 'N/A')

            row_data = [
                hostname,
                ip,
                port,
                description,
                status,
                vlan,
                neighbor,
                port_speed,
                link_uptime,
                device_serial,
                ", ".join(power_supply_serials),
                transceiver_info
            ]
            sheet_db.append(row_data)

        net_connect.disconnect()
        print(f"Successfully collected data from {ip}")

    except Exception as e:
        print(f"Failed to connect to {ip}: {e}")
        # Add a row indicating the failure
        sheet_db.append([ "CONNECTION FAILED", ip, str(e), "", "", "", "", "", "", "", ""])

    # Wait for 30 seconds before connecting to the next device
    print("Waiting for 30 seconds...")
    time.sleep(30)

# Save the workbook
wb_db.save('DataBase.xlsx')

print("Script finished. Data saved to DataBase.xlsx")
